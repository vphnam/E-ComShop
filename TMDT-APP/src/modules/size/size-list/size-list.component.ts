...
export class SizeListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.shared.getData().subscribe(data=>{
      if(data != null)
      {
        this.refreshSizeListBySearchForm();
      }
      else
      {
        console.warn("here");
  ...
  refreshSizeListBySearchForm() {
    this.http.get("/Size").subscribe(data => {
      console.warn(data);
