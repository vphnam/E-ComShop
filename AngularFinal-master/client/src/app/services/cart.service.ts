...
export class CartService {
  ...
  addProduct(params): void {
    const { serialNo, price, quantity, image, title, maxQuantity } = params;
    const product = { serialNo, price, quantity, image, title, maxQuantity };

    if (!this.isProductInCart(serialNo)) {
      ...
      let productIndex = updatedProducts.findIndex((prod) => prod.serialNo == serialNo);
  ...
  updateCart(serialNo: number, quantity: number): void {
    // copy array, find item index and update
    let updatedProducts = [...this.cartData.products];
    let productIndex = updatedProducts.findIndex((prod) => prod.serialNo == serialNo);
  ...
  removeProduct(serialNo: number): void {
    let updatedProducts = this.cartData.products.filter(
      (prod) => prod.serialNo !== serialNo
    );
    this.cartData.products = updatedProducts;
    this.cartData.total = this.getCartTotal();
    this.cartDataObs$.next({ ...this.cartData });
  ...
  isProductInCart(serialNo: number): boolean {
    return this.cartData.products.findIndex((prod) => prod.serialNo === serialNo) !== -1;
  }
}
